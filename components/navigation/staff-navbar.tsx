...
import { ClipboardList, Menu, BarChart3 } from "lucide-react"
...
export function StaffNavbar({ activeTab, onTabChange, orderCount = 0 }: StaffNavbarProps) {
  const [isAnimating, setIsAnimating] = useState(false)
  const [prevOrderCount, setPrevOrderCount] = useState(orderCount)
  ...
  const navItems = [
    { id: "orders", icon: ClipboardList, label: "Orders", badge: orderCount },
  ...
  return (
    <>
      ...
      >
        <div className="flex justify-center px-2 sm:px-3 md:px-5 lg:px-6">
          <div
            className="bg-white rounded-2xl sm:rounded-2xl md:rounded-3xl border border-gray-100/60 px-2 sm:px-3 lg:px-5 py-3 sm:py-3.5 md:py-4 max-w-sm sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl w-auto md:px-5"
            style={{
              boxShadow:
                    ...
                    <Tooltip key={item.id}>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          aria-label={`${item.label} tab${isActive ? ", currently selected" : ""}${item.badge ? `, ${item.badge} orders` : ""}`}
                          aria-current={isActive ? "page" : undefined}
                        >
                          <Icon
                            className={`w-4 h-4 sm:w-5 sm:h-5 md:w-5 md:h-5 lg:w-6 lg:h-6 transition-transform duration-200 ${isActive ? "scale-110" : "group-hover:scale-105"}`}
